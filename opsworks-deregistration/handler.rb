...
def lambda_handler(args)
  ...
  if message['Event'].eql? 'autoscaling:EC2_INSTANCE_TERMINATE'
    ec2client = Aws::EC2::Client.new(ENV['AWS_DEFAULT_REGION'])
    
    instance_id = message['EC2InstanceId']
    puts "Removing instance from OpsWorks: #{instance_id}"

    ec2client.describe_instances(instance_ids: [instance_id]).reservations[0].tags.each do |tag|
      if tag.key.eql? 'opsworks_stack_id'
        stack_id = tag.value
        puts "Checking OpsWorks Stack: #{stack_id}"
        opsworks_client = AWS::OpsWorks::Client.new(ENV['AWS_DEFAULT_REGION'])
